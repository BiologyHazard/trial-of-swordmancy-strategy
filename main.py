...
class MDPResult(NamedTuple):
    ...
    def 导出CSV(self, 文件路径: str = "strategy.csv") -> None:
        """将最优策略导出为 CSV 文件"""
        import csv
    ...
    def 打印最优路径(self, 求解器: 求解器类) -> None:
        ...
        步骤 = 0
        累积奖励 = 0.0
        print("最优策略路径（从初始状态开始）:")
        while not isinstance(当前状态, 吸收态类):
            序号 = self.状态索引[当前状态]
            决策 = self.最优策略[序号]
            if 决策 is None:
                break
            即时奖励 = 求解器.行动奖励(状态=当前状态, 决策=决策)
            转移 = 求解器.状态转移(起始状态=当前状态, 决策=决策)
            步骤 += 1
            累积奖励 += 即时奖励
            手牌 = (
                当前状态.已抽到的铭牌数量元组 if isinstance(当前状态, 过渡态类) else ()
            )
            轮次_s = (
                str(当前状态.剩余演算次数) if isinstance(当前状态, 过渡态类) else "-"
            )
            放弃_s = (
                str(当前状态.剩余放弃次数) if isinstance(当前状态, 过渡态类) else "-"
            )
            翻倍_s = (
                str(当前状态.剩余翻倍次数) if isinstance(当前状态, 过渡态类) else "-"
            )
            翻倍状态_s = (
                当前状态.已确定翻倍状态.name if isinstance(当前状态, 过渡态类) else "-"
            )
            print(
                f"  步骤{步骤:2d}: {决策.name:8s}  "
                f"(轮次={轮次_s}, 放弃={放弃_s}, 翻倍={翻倍_s}, "
                f"状态={翻倍状态_s}, 手牌={手牌})  "
                f"奖励={即时奖励:>8.0f}  累积={累积奖励:>10.2f}"
            )
            if len(转移) == 1:
                当前状态, _ = 转移[0]
            else:
...
class 求解器类:
    ...
    def 计算演算奖励(
        self, 已抽到的铭牌数量元组: tuple[int, ...], 已确定翻倍状态: 翻倍状态类
    ) -> float:
        ...
        match 决策:
            ...
            case 决策空间类.开始演算:
                目标状态 = self.获取状态(
                    ...
                    剩余放弃次数=起始剩余放弃次数,
                    剩余翻倍次数=起始剩余翻倍次数
                    - (1 if 起始已确定翻倍状态 == 翻倍状态类.翻倍 else 0),
                    已确定翻倍状态=翻倍状态类.未确定,
                    已抽到的铭牌数量元组=(0, 0, 0, 0, 0),
                )
                转移概率列表.append((目标状态, 1.0))
    ...
    def 状态容许决策(self, 状态: 状态类) -> list[决策空间类]:
        ...
        if 已抽到的铭牌总数量 == 2 and 状态.已确定翻倍状态 == 翻倍状态类.未确定:
            # 已抽到的铭牌总数量为 2 且未确定翻倍状态时，必须选择翻倍或不翻倍
            if 状态.剩余翻倍次数 == 0:
                return [决策空间类.不翻倍]
            else:
                return [决策空间类.不翻倍, 决策空间类.翻倍]

        elif 已抽到的铭牌总数量 == 5:
            return [决策空间类.开始演算, 决策空间类.放弃]

        else:
            return [决策空间类.抽取铭牌, 决策空间类.放弃, 决策空间类.开始演算]
    ...
    def 是需要考虑的状态(self, 状态: 过渡态类) -> bool:
        if not (
            1 <= 状态.剩余演算次数 <= 3
            and 0 <= 状态.剩余放弃次数 <= 3
            # and 状态.剩余演算次数 - 1 <= 状态.剩余翻倍次数 <= 状态.剩余演算次数
            and 状态.剩余演算次数 - 1 <= 状态.剩余翻倍次数 <= 2
        ):
            return False

        已抽到的铭牌总数量 = sum(状态.已抽到的铭牌数量元组)
        if 已抽到的铭牌总数量 <= 1 and 状态.已确定翻倍状态 is not 翻倍状态类.未确定:
            # 已抽到的铭牌总数量 <= 1 时，不能已确定翻倍状态
            return False
        if 已抽到的铭牌总数量 >= 3 and 状态.已确定翻倍状态 is 翻倍状态类.未确定:
            # 已抽到的铭牌总数量 >= 3 时，必须已确定翻倍状态
            return False
...
if __name__ == "__main__":
    ...
    print("\n部分状态的最优策略示例:")
    result.打印最优策略(最多显示=100000000)

    result.导出CSV("strategy.csv")
